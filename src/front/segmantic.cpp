...
// 类型转换
Operand frontend::Analyzer::convert_if_needed(Operand src, Type target, vector<Instruction*>& buffer) {
    if (src.type == target) return src;

    // 处理字面量到变量的转换
    if (src.type == Type::IntLiteral && target == Type::Int) {
        Operand res("t" + std::to_string(tmp_cnt++), Type::Int);
        buffer.push_back(new Instruction(src, {}, res, Operator::def));
        return res;
    }
    if (src.type == Type::FloatLiteral && target == Type::Float) {
        Operand res("t" + std::to_string(tmp_cnt++), Type::Float);
        buffer.push_back(new Instruction(src, {}, res, Operator::fdef));
        return res;
    }

    // 处理类型转换
    if (src.type == Type::Int && target == Type::Float) {
        Operand res("t" + std::to_string(tmp_cnt++), Type::Float);
        buffer.push_back(new Instruction(src, {}, res, Operator::cvt_i2f));
        return res;
    }
    if (src.type == Type::Float && target == Type::Int) {
        Operand res("t" + std::to_string(tmp_cnt++), Type::Int);
    ...
    // 处理字面量之间的转换
    if (src.type == Type::IntLiteral && target == Type::Float) {
        return Operand(std::to_string(std::stof(src.name)), Type::FloatLiteral);
    }
    if (src.type == Type::FloatLiteral && target == Type::Int) {
        return Operand(std::to_string((int)std::stof(src.name)), Type::IntLiteral);
    }
...
void frontend::Analyzer::type_transform(Operand& a, Operand& b, vector<Instruction*>& buffer){
    if (a.type == Type::Int){
        ...
        if (b.type == Type::Int){
            ...
            // a = tmp_op;
            // FloatLiteral与Float运算
            auto tmp = Operand("t" + std::to_string(tmp_cnt++), Type::Float);
            buffer.push_back(new Instruction(a, {}, tmp, Operator::fdef));
            a = tmp;
        }
    }
}
...
Operand frontend::SymbolTable::get_operand(string id) const {
    ...
    assert(false && "Operand not found in symbol table");
    return {}; 
}
...
frontend::STE frontend::SymbolTable::get_ste(string id) const {
    ...
    assert(false && "STE not found in symbol table");
    return {}; // 同上
}
...
void frontend::Analyzer::analysisStmt(Stmt* root, vector<ir::Instruction*>& buffer){
    ...
    if ((int)root->children.size() == 2){
        ...
        if (curr_function->returnType == Type::Int) {
            // 确保返回类型是Int
            Operand ret_val = convert_if_needed(Operand(exp->v, exp->t), Type::Int, buffer);
            Instruction* return_inst = new Instruction(ret_val, Operand(), Operand(), Operator::_return);
            buffer.push_back(return_inst);
        }
        else if (curr_function->returnType == Type::Float) {
            // 确保返回类型是Float
            Operand ret_val = convert_if_needed(Operand(exp->v, exp->t), Type::Float, buffer);
            Instruction* return_inst = new Instruction(ret_val, Operand(), Operand(), Operator::_return);
            buffer.push_back(return_inst);
        }
        else {
